-- CORRECTED DATABASE SCHEMA FOR SUPABASE
-- This exactly matches the API function calls
...
-- Step 4: Create the EXACT unique constraints that the API expects
CREATE UNIQUE INDEX idx_officials_unique_constraint ON officials(state, name, office);
CREATE UNIQUE INDEX idx_counties_unique_constraint ON counties(full_fips);
...
-- Step 8: Create upsert_official function with EXACT parameters from API
...
-- Step 9: Create upsert_county function with EXACT parameters from API
...
-- Step 10: Insert Pennsylvania officials data
